...
$script:appName = "appAvgTimeEntryDuration"
$FrequencyMinutes = 2
...
function New-CWMAvgTimeEntryDurationReport {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory,
            ValueFromPipeline)]
        ...
        [string]$CSVPath = (Get-Location).Path + "\CWMAvgTimeEntryDurationReport.csv",

        [ValidatePattern('([0-9]|[A-Z])\.html$')]
        [Parameter(HelpMessage = "Enter a full file path and name ending in .html")]
        [string]$HTMLPath = (Get-Location).Path + "\CWMAvgTimeEntryDurationReport.html",
    ...
    PROCESS {
        ...
        $TimeEntryDuration = Get-CWMAvgTimeEntryDuration -TicketID $TicketID
        
        $obj = [PSCustomObject]@{
            TicketID             = $BaseStat.id
            Company              = $BaseStat.company.name
            Contact              = $BaseStat.contact.name
            AvgTimeEntryDuration = $TimeEntryDuration.duration ## property specific to this report
            TimeEntryCount       = $TimeEntryDuration.count ## property specific to this report
...
while ($true) {
    ...
    try {
        ...
        $Id | New-CWMAvgTimeEntryDurationReport `
            -CSVPath "$dataPath/appAvgTimeEntryDuration.csv" `
            -HTMLPath "$dataPath/appAvgTimeEntryDuration.html"
