...
$script:appName = "appAvgTimeEntryGap"
$FrequencyMinutes = 2
...
function New-CWMAvgTimeEntryGapReport {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory,
            ValueFromPipeline)]
        ...
        [string]$CSVPath = (Get-Location).Path + "\CWMAvgTimeEntryGapReport.csv",

        [ValidatePattern('([0-9]|[A-Z])\.html$')]
        [Parameter(HelpMessage = "Enter a full file path and name ending in .html")]
        [string]$HTMLPath = (Get-Location).Path + "\CWMAvgTimeEntryGapReport.html",
    ...
    PROCESS {
        ...
        $TimeEntryGapDays = Get-CWMAvgTimeEntryGap -TicketID $TicketID
        
        $obj = [PSCustomObject]@{
            TicketID        = $BaseStat.id
            Company         = $BaseStat.company.name
            Contact         = $BaseStat.contact.name
            AvgTimeEntryGap = $TimeEntryGapDays ## property specific to this report
    ...
    END {
        $ObjCollection | Sort-Object -Property AvgTimeEntryGap -Descending | Select-Object -First $ItemsToDisplay | Export-Csv -Path $CSVPath -NoTypeInformation -Force
        $ObjCollection | Sort-Object -Property AvgTimeEntryGap -Descending | Select-Object -First $ItemsToDisplay | ConvertTo-Html -CssUri reportstyle.css | Out-File -FilePath $HTMLPath -Force
    }
}
...
while ($true) {
    ...
    # Generate appAvgTimeEntryGap report
    try {
        New-CWMLog -Type "Info" -Message "Generating report $script:appName..."
        $Id | New-CWMAvgTimeEntryGapReport `
            -CSVPath "$dataPath/appAvgTimeEntryGap.csv" `
            -HTMLPath "$dataPath/appAvgTimeEntryGap.html" `
            -ItemsToDisplay 40
